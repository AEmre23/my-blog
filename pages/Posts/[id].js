...
const Id = () => {
  ...
  const deletePost = async () => {
    if (isAuthor | user.email === "ahmetemre1998@hotmail.com"){
      const postDoc = doc(db, "posts", pagePost?.id)
      await deleteDoc(postDoc)
      console.log("Post Deleted")
      router.push("/Posts")
      setTimeout(() => { window.location.reload() }, 1000)
    } else throw new Error("You don't have permisson to delete.")
  }
  ...
  return (
    <Layout>
      <main>
        ...
        :
        <div className="p-8 mobile:py-4 mobile:px-2 shadow-md w-full mb-12 flex mobile:flex-col items-start gap-2 border-newtext border-2 rounded bg-white/90">
          <div className="flex flex-col gap-3 w-3/4 mobile:w-full font-rubik ">
            <div className="border-2 mobile:mb-2 border-newtext text-center text-lg p-2 rounded-full px-4 hidden mobile:block">{pagePost?.category}</div>
            ...
            <div className="flex mobile:flex-col-reverse pb-6 mobile:-mt-5 items-center justify-between">
              ...
              <div className="border-2 mobile:mb-4 border-newtext text-lg p-2 rounded-full px-4 mobile:hidden">{pagePost?.category}</div>
            </div>
          ...
          <div className={`${isDetailOpen ? "mb-4" : ""} hidden mobile:block p-2 mt-2 border-2 w-full border-newtext rounded`}> {/*  Mobile Detail Section  */}
            ...
            <div ref={openable} className={`max-h-0 overflow-hidden duration-300 flex flex-col gap-3`}>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {(isAuthor | user.email === "ahmetemre1998@hotmail.com") ?
                  ...
                  <button className="px-3 py-1 bg-white border-2 border-newtext mt-2 rounded-lg text-newtext duration-200 hover:bg-newtext hover:text-white ">Edit this post</button>
                    </Link>
