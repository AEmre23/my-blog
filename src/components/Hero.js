...
import goDown from '../assets/arrow.png'
  ...
  return (
    <main>
      <div className="min-h-screen w-full bg-[url('/bg.png')] bg-[size:100%_100%] mobile:bg-[size:160%_110%] bg-no-repeat relative font-rubik cursor-default">
        <div className="absolute right-[10%] bottom-[33%] bigscreen:right-[10%] mobile:bottom-0 mobile:top-28 mobile:right-0 mobile:left-4">
          <h1 className="text-5xl bigscreen:text-6xl font-bold mobile:max-w-[300px]">What's on your mind?</h1>
          <h2 className= "text-4xl mobile:text-[33px] bigscreen:text-5xl font-medium mt-7 mb-16 mobile:mb-8 mobile:max-w-[300px] mobile:ml-0">Share with everyone!</h2>
          ...
          <Link href={"/Posts"}>
            <button className=" p-2 px-4 bg-newbg mobile:bg-scolor mobile:text-newbg duration-300 hover:bg-newtext hover:text-newbg hover:scale-105 font-bold text-xl rounded-lg shadow-2xl">SEE BLOG POSTS</button>
          </Link>
        </div>
        <a href="#recent">
          <div className="absolute cursor-pointer -bottom-4 left-1/2 -translate-x-1/2 mobile:bottom-16 mobile:left-44 mobile:translate-x-0 duration-200 scale-75 hover:-rotate-6">
            <Image className="pointer-events-none" src={goDown} alt="scroll-down" />
          </div>
        </a>
      ...
      <div id="recent" className="relative min-h-screen w-full bg-[url('/bgr.png')] bg-[size:100%_100%] bg-no-repeat mobile:bg-[size:100%_25%] mt-0">
